		...
		for i2 in range(i1+1,n-2):
			for i3 in range(i2+1,n-1):
				for i4 in range(i3+1,n):
					combo = G[i1] + " " + G[i2] + " " + G[i3] + " " + G[i4]   
					combo2 = G[i1] + "_" + G[i2] + "_" + G[i3] + "_" + G[i4]   
...
rule Intersect:
	...
	output:
		tsv = expand("{RES_DIR}/{{SP4}}_{{SP3}}_{{SP2}}_{{SP1}}/{{SP4}}_{{SP3}}_{{SP2}}_{{SP1}}.tsv",RES_DIR=config["RES_DIR"])

	conda:
		config["CONDA_FILE2"]
	shell:
		"""Rscript {config[CODE_DIR]}/Intersect4.R {wildcards.SP1} {wildcards.SP2} {wildcards.SP3} {wildcards.SP4} {output.tsv} """
