...
snakemake -s ~/HGTnew/multi_comparisons/ThreeFold/src/mobilome/1.bis.construct_mobilomes_entero.smk -n --unlock

snakemake -s ~/HGTnew/multi_comparisons/ThreeFold/src/mobilome/1.bis.construct_mobilomes_entero.smk \
           --use-conda \
	   --cluster-config ~/HGTnew/multi_comparisons/ThreeFold/src/config_sge.yml \
	   --cluster "sbatch -N 1 -c 1 -J ENTERO  -o $LOGDIR/%j.log -t {cluster.time} --mem {cluster.mem}" \
	   --jobs 15 \
           --configfile ~/HGTnew/multi_comparisons/ThreeFold/src/mobilome/config_mobilome_entero.yml \
	   --rerun-incomplete 
