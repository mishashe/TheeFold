...
parser.add_option("-m","--match_file",dest="match_file",action="store",
help="the directory containing all strains of the clade we want to analyse")
...
parser.add_option("-a", "--acc_file",dest="acc_file",action="store",
help="correspondance between accession and file name")
...
Matches = {}
mm = open (match_file,'r')
for line in mm.readlines():
	line = line.rstrip()
	row = line.split("\t")
	row[0] = row[0][1:]
	if row[0] not in Matches.keys():
		Matches[row[0]] = []

	Matches[row[0]].append([int(row[1]),int(row[1])+int(row[3])])
...
for cle in Matches.keys():
	...
	for index in range(0,len(Matches[cle])):
		print(str(index)+" -- "+str(len(Matches[cle])))
		opFile.write(">"+cle+"--"+str(Matches[cle][index][0])+"-"+str(Matches[cle][index][1])+"\n")
#			contig_len = len(all_contigs[cle])
		start = int(Matches[cle][index][0])
		end = int(Matches[cle][index][1])
...
opFile.close()
comm = "blastn -subject "+output_file+" -query "+db_file+" -outfmt 6 -out "+output_file+"_blastn"
os.system(comm)

if pdb_file != "NO":
	commP = 'tblastn -subject '+output_file+' -query '+pdb_file+' -outfmt 6 -out '+output_file+"_tblastn"
	print(commP)
	os.system(commP)


#subprocess.run(['blastn','-subject',output_file,'-query',db_file,'-outfmt','6','-out'], stdout=subprocess.PIPE).stdout.decode('utf-8')
#
