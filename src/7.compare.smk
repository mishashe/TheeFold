...
def get_combos(Genus_list):
	...
	G = Genus_list
	n = len(G)
	for i1 in range(0,n-6):
		for i2 in range(i1+1,n-5):
			for i3 in range(i2+1,n-4):
				for i4 in range(i3+1,n-3):
					for i5 in range(i4+1,n-2):
						for i6 in range(i5+1,n-1):
							for i7 in range(i6+1,n):
								combo = G[i1] + " " + G[i2] + " " + G[i3] + " " + G[i4] + " " + G[i5] + " " + G[i6] + " " + G[i7]
								combo2 = G[i1] + "_" + G[i2] + "_" + G[i3] + "_" + G[i4] + "_" + G[i5] + "_" + G[i6] + "_" + G[i7]
...
rule Intersect:
	...
	output:
		tsv = expand("{RES_DIR}/{{SP7}}_{{SP6}}_{{SP5}}_{{SP4}}_{{SP3}}_{{SP2}}_{{SP1}}/{{SP7}}_{{SP6}}_{{SP5}}_{{SP4}}_{{SP3}}_{{SP2}}_{{SP1}}.tsv",RES_DIR=config["RES_DIR"])

	conda:
		config["CONDA_FILE2"]
	shell:
		"""Rscript {config[CODE_DIR]}/Intersect7.R {wildcards.SP1} {wildcards.SP2} {wildcards.SP3} {wildcards.SP4} {wildcards.SP5} {wildcards.SP6} {wildcards.SP7} {output.tsv} """
